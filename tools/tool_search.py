...
def _bm25_ranked(
    ...
    Why: Separating scoring from slicing keeps the ranked list reusable and
    the public ``search_catalog`` slice trivial.
...
def dispatch_tool_search(args: Dict[str, Any],
    ...
    hits = search_catalog(catalog, query, limit=limit, config=config)
    return json.dumps({
        "query": query,
