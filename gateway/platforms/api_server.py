        ...
        *route* is an optional ``model_routes`` entry (resolved from the
        ...
        def _run():
            ...
            try:
                ...
                # Home + secret scope together; None is a no-op.
            ...
            try:
                ...
                def _run_sync():
                    ...
                    try:
                        ...
                        # Bind the routed agent profile (home + fail-closed secret
                        # scope) inside the executor thread — asyncio's default
                        # executor does not copy ContextVars.  Construction and
                        # execution share one guard, matching _run_agent.
                        with _use_profile_and_secret_scope(agent_profile):
                            agent = self._create_agent(
                                ephemeral_system_prompt=ephemeral_system_prompt,
                                session_id=session_id,
