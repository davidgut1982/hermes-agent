...
def _current_cron_store() -> _CronStorePaths:
    ...
    Resolution order:
    1. An explicit ``use_cron_store()`` override (cross-profile scoping).
    2. Otherwise the dynamic ``_get_cron_dir()`` family, which honors a
       test-monkeypatched ``CRON_DIR`` and, failing that, the active
       ``AgentProfile``'s ``get_hermes_home()``. This makes the multi-agent
    ...
    if override is not None:
        return override
    return _CronStorePaths(_get_cron_dir(), _get_jobs_file(), _get_output_dir())
