...
def _get_event_state(app: "FastAPI") -> Tuple[Dict[str, set], asyncio.Lock]:
    ...
    when the lifespan handler has not populated ``app.state`` yet (e.g. a
    TestClient constructed without a ``with`` block). The lifespan path is
    preferred because it guarantees the Lock is created on the correct loop.
