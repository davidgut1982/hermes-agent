...
- **feat/tool-search-hybrid-rerank** — Optional embedding reranker for progressive tool disclosure; hybrid BM25 + semantic search for MCP tool selection, dramatically reduces tool hallucination. (The `EmbeddingReranker` and related components were subsequently extracted from `tool_search.py` to the eval suite's `run_live_benchmark.py`; production now uses pure BM25 — see the `no-mistakes(review)` entry below.)
